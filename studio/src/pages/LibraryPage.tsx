      ...
      <div className="resource-list">
        {unit.watch.map(([url, title, focus], index) => (
          <article className="resource-row" key={url}>
            ...
            {index === 0 ? <Link className="button button--secondary" to="/session">Open <ExternalLink size={17} aria-hidden="true" /></Link> : <a className="button button--secondary" href={url} target="_blank" rel="noreferrer">Open <ExternalLink size={17} aria-hidden="true" /></a>}
          </article>
        ))}
      </div>
    </section>
